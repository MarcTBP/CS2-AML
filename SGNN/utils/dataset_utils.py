...
def get_all_np_list():
    ...
    np.savez(addr_feature_np_list_path, addr_features_train_list=np.array(addr_features_list[:34], dtype=object),
             addr_features_test_list=np.array(addr_features_list[34:], dtype=object))
    np.savez(addr_class_np_list_path, addr_classes_list=np.array(addr_classes_list, dtype=object))

    np.savez(tx_np_list_path,
             txs_features_list=np.array(txs_features_list, dtype=object), txs_classes_list=np.array(txs_classes_list, dtype=object), txs_edges_list=np.array(txs_edges_list, dtype=object))

    np.savez(hyperedge_np_list_path,
             hyperedge_index_list=np.array(hyperedge_index_list, dtype=object), hyperedge_weight_list=np.array(hyperedge_weight_list, dtype=object))
    ...
    np.savez(addr_edge_np_list_path, addrTxValue_list=np.array(addrTxValue_list, dtype=object), txAddrValue_list=np.array(txAddrValue_list, dtype=object),
             addr_edge_index_list=np.array(addr_edge_index_list, dtype=object))
    np.savez(idx_np_list_path, addr_idx_list=np.array(addr_idx_list, dtype=object), txs_idx_list=np.array(txs_idx_list, dtype=object))
